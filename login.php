 ...
 if($_SERVER['REQUEST_METHOD']== "POST")
  {
    ...
    if(!empty($firstName) && !empty($lastName)&& !empty($password))
    {
        ...
        $query = "select * from registration where First_name='$First_name' limit 1";
...
<Doctype! Html>
  ...
  .reg{
      margin:auto;
background-color:rgb(16, 142, 214);
max-width: 450px;
padding:50px;
...
input[type=submit]{
  background-color: rgb(132, 202, 235);
  width: 100%;
  padding: 12px 20px;
  box-sizing: border-box;
  border-radius:12px;
...
input[type=submit]:hover{
  background-color: rgb(4, 121, 101);
  width: 100%;
  padding: 12px 20px;
  box-sizing: border-box;
  border-radius:12px;
...
<div class="reg">
  <p id="nice">INJIRA<p>
    <form method="post">
   <input type="text" name="First_name" placeholder="izina ribanza"/input><br>
   <input type="password" name="user_password" placeholder="ijambo banga" /input><br>
  <input type="submit" value="EMEZA">
  <a href="signup.php">IYANDIKISHE</a>
  <form>
 </div>
